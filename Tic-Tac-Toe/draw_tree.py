...
class TreeDisplay:
    ...
    def get_box_size(self, tree):
        ...
        for child in tree.children():   # base case is included in here implicitly
            subtree_width, subtree_height = self.get_box_size(child)
            max_subtree_height = max(max_subtree_height, subtree_height)
            current_width += subtree_width
        tree_width = max(current_width, NODE_WIDTH)
        tree_height = NODE_HEIGHT + max_subtree_height
